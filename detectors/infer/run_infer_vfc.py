...
def run1(library_name, opt, filename, full_check, detector_name):
    ...
    if library_name == "tensorflow":
        ...
        if full_check and detector_name == "clang":
            ...
            command = (
                "clang-tidy " + _path + " -checks=-*,clang-analyzer-* -- " + command_
            )
        ...
        if (
            ...
            try:
                ...
                if full_check and detector_name == "infer":
                    ...
                    command_capture = (
                        "infer --keep-going -- gcc " + opt["command"] + " " + filename
                    )
                    command_analyze = (
                        "infer analyze -- gcc " + opt["command"] + " " + filename
                    )
            ...
            for i, dir in enumerate(os.listdir(vic_path)):
                ...
                try:
                    for counter, item in enumerate(data):
                        _id += 1
                        x = list(item.keys())
                        current_commit = PyDrillerGitRepo(repository_path).get_commit(
                            x[0]
                        )
                        for mod in current_commit.modifications:
                            if (
                                "test" not in str(mod.old_path)
                                and "test" not in str(mod.new_path)
                                and "test" not in mod.filename
                                and mod.filename.split(".")[-1] in _extensions
                            ):
                                ...
                                if mapping_ == "TrueBugs" and opt:
                                    (
                                        ...
                                        cl, mod, tool, user_names[i], opt[0], full_check
                                    )
                                    if res == "not detected":
                                        ...
                                        my_data = [
                                            ...
                                            vul_file_object.added,
                                            vul_file_object.removed,
                                            0,
                                        ]
                                        ...
                                        for v in range(len(res[1])):
                                            ...
                                            with open(
                                                "./detection_results/infer_fullcheck/vul_frequency.csv",
                                                "a",
                                                newline="\n",
                                            ) as fd:
                                    ...
                                    with open(
                                        "./detection_results/infer_fullcheck/results_true.csv",
                                        "a",
                                        newline="\n",
                                    ) as fd:
                                ...
                                if mapping_ == "diff" and opt:
                                    ...
                                    if res == "not detected":
                                        ...
                                        with open(
                                            "./detection_results/infer_fullcheck/change_info.csv",
                                            "a",
                                            newline="\n",
                                        ) as fd:
                                            writer_object = writer(fd)
                                    ...
                                    with open(
                                        "./detection_results/infer_fullcheck/results.csv",
                                        "a",
                                        newline="\n",
                                    ) as fd:
                                        writer_object = writer(fd)
                                ...
                                if mapping_ == "fixed" and opt:
                                    ...
                                    if flag:
                                        ...
                                        with open(
                                            "./detection_results/infer_fullcheck/change_info_fixed.csv",
                                            "a",
                                            newline="\n",
                                        ) as fd:
                                            writer_object = writer(fd)
                                        ...
                                        for v in range(len(wfix)):
                                            ...
                                            with open(
                                                "./detection_results/vul_frequency.csv",
                                                "a",
                                                newline="\n",
                                            ) as fd:
                                                writer_object = csv.writer(fd)
                                    ...
                                    with open(
                                        "./detection_results/infer_fullcheck/results_fixed.csv",
                                        "a",
                                        newline="\n",
                                    ) as fd:
                                        writer_object = writer(fd)
                                ...
                                if mapping_ != "fixed" and not opt:
                                    with open(
                                        "./detection_results/infer_fullcheck/compile_options_failed.csv",
                                        "a",
                                        newline="\n",
                                    ) as fd:
                                        writer_object = writer(fd)
                                ...
                                with open(
                                    "./detection_results/infer_fullcheck/filtered_files.csv",
                                    "a",
                                    newline="\n",
                                ) as fd:
                                    writer_object = writer(fd)
