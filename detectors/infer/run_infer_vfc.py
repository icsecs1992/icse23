...
def fixed_warning_base_matching(fix_commit, vul_commit, detector_name, library_name, opt, full_check, current_commit):
        ...
        if bool(wfixed):
            for w in wfixed:
                if REG_LOC_INFER.search(res1[0][w][0]):
                    cwe_final_list = cwe_final_list + [REGEX_INFER.search(res1[0][w][0]).group(1)]
    ...
    return flag, vul_commit, res1, res2, execution_time1+execution_time2, cwe_final_list
                                ...
                                if 'test' not in str(mod.old_path) and 'test' not in str(mod.new_path) and 'test' not in mod.filename and mod.filename.split('.')[-1] in _extensions:
                                    ...
                                    if mapping_ == 'TrueBugs' and opt:
                                        ...
                                        if res == 'not detected':
                                            ...
                                            for v in range(len(res[1])):
                                                ...
                                                with open('./detection_results/infer_fullcheck/vul_frequency.csv', 'a', newline='\n') as fd:
                                                    writer_object = csv.writer(fd)
                                                    writer_object.writerow(vul_freq_data)

                                        with open('./detection_results/infer_fullcheck/results_true.csv', 'a', newline='\n') as fd:
                                    ...
                                    if mapping_ == 'diff' and opt:
                                        ...
                                        if res == 'not detected':
                                            ...
                                            with open('./detection_results/infer_fullcheck/change_info.csv', 'a', newline='\n') as fd:
                                                writer_object = writer(fd)
                                                writer_object.writerow(cl_list)

                                                    
                                        with open('./detection_results/infer_fullcheck/results.csv', 'a', newline='\n') as fd:
                                                writer_object = writer(fd)
                                                writer_object.writerow(my_data)

                                    if mapping_ == 'fixed' and opt:
                                        flag, vul_file_object, res1, res2, execution_time, wfix = fixed_warning_base_matching(cl, mod, tool, user_names[i], opt[0], full_check, current_commit)
                                        ...
                                        if flag:
                                            ...
                                            with open('./detection_results/infer_fullcheck/change_info_fixed.csv', 'a', newline='\n') as fd:
                                                    writer_object = writer(fd)
                                                    writer_object.writerow(cl_list)

                                            for v in range(len(wfix)):
                                                vul_freq_data = [tool, dir.split('_')[1].split('.')[0]]
                                                vul_freq_data = vul_freq_data + [wfix[v]]
                                                vul_freq_data = [_id] + vul_freq_data
                                        ...
                                        with open('./detection_results/infer_fullcheck/results_fixed.csv', 'a', newline='\n') as fd:
                                            writer_object = writer(fd)
                                            writer_object.writerow(my_data)

                                    if mapping_ != 'fixed' and not opt:
                                        with open('./detection_results/infer_fullcheck/compile_options_failed.csv', 'a', newline='\n') as fd:
                                            writer_object = writer(fd)
                                            writer_object.writerow([dir.split('_')[1].split('.')[0], x[0], mod.new_path, mod.filename])
                                else:
                                    with open('./detection_results/infer_fullcheck/filtered_files.csv', 'a', newline='\n') as fd:
                                        writer_object = writer(fd)
                                        writer_object.writerow([dir.split('_')[1].split('.')[0], x[0], mod.new_path, mod.filename])
