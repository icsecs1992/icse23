...
def fixed_warning_base_matching(fix_commit, vul_commit, detector_name, library_name, mapping_, cwe_target, current_commit):
    ...
    if os.path.isfile(this_project+'/fix_'+vul_commit.filename):
        [output2, execution_time2] = run(this_project+'/fix_'+vul_commit.filename, detector_name, library_name)
    ...
    if not isinstance(res1, str) and not isinstance(res2, str):
        ...
        if bool(wfixed):
            for w in wfixed:
                if detector_name == 'flawfinder':
                    cwe_list = find_regex_groups(res1[0][w][0])
                    out = out + cwe_list
                if detector_name == 'cppcheck':
                    cwe_list = find_cppcheck_cwe(res1[0][w][0])
                    out = out + cwe_list
                if detector_name == 'rats':
                    cwe_list = find_rat_types(res1[0][w][0])
                    out = out + cwe_list
                    ...
                    for counter, item in enumerate(data):
                            ...
                            for mod in current_commit.modifications:
                                if 'test' not in str(mod.old_path) and 'test' not in str(mod.new_path) and 'test' not in mod.filename and mod.filename.split('.')[-1] in _extensions:
                                    ...
                                    if mapping_ == 'fixed':
                                        flag, vul_file_object, res1, res2, execution_time, wfix = fixed_warning_base_matching(cl, mod, tool, user_names[i], mapping_, label_dict[x[0]], current_commit)
                                        ...
                                        if flag:
                                            ...
                                            with open('./detection_results/change_info_fix.csv', 'a', newline='\n') as fd:
                                                    writer_object = csv.writer(fd)
                                                    writer_object.writerow(cl_list)

                                        with open('./detection_results/results_fix.csv', 'a', newline='\n') as fd:
                                            writer_object = csv.writer(fd)
                                            writer_object.writerow(my_data)

                                else:
                                    with open('./detection_results/filtered_files_fix.csv', 'a', newline='\n') as fd:
                                        writer_object = csv.writer(fd)
