...
def get_commits(githubUser, currentRepo, qm, page, amp, sh_string, last_com , page_number, start_date, ending_date, branch_sha, token):
  ...
  for i, commit in enumerate(first_100_commits):
    ...
    if i == len(first_100_commits)-1:
        last_com = commit['sha']
        get_commits(githubUser, currentRepo, qm, page, amp, sh_string, last_com , page_number, start_date, ending_date, branch_sha, token)
...
def main(args):
  ...
  try:
    get_commits(args.user, args.repo, qm, page, amp, sh_string, last_com , page_number, args.start_date, args.ending_date, branch_sha, args.gitToken)
  except:
...
if __name__ == "__main__":
  ...
  parser.add_argument('--ending_date', default=now.year , type=int, help='Please specify an ending date for commit collection.')
